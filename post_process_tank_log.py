...
def create_plots(SV_idx, sim_outputs, tank, solids, params, total_volume,time_end,rxn_rates ,q_dot,S_8_disolve_rate,Li2S_disolve_rate, plot_flags):
    '''
    ...
    # One plot for each of the elyte species concentrations
    if plot_flags[1] == 1:
        ...
        plt.suptitle(r"Species Concentration")
        plt.tight_layout()
    ...
    if plot_flags[3] == 1:
        ...
        ax6.legend()
        plt.suptitle(r"Conservation of Moles")
        plt.tight_layout()
    ...
    # Plot the gibs free energies for each reaction
    if plot_flags[5] == 1:
        for t in range(np.size(C_k_elyte,1)):
                for i, el in enumerate(nu):
                    ...
                    while math.isnan(conc_adjustment[t,i]) or math.isinf(conc_adjustment[t,i]) or conc_adjustment[t,i]==0:
                        ...
                        conc_adjustment[t,i] = ct.gas_constant * tank.elyte_obj.T/1e6*np.log(np.prod(np.power(X_k_timestep_capped,el)))
                        min_C = min_C*2
                ...
                g_f_solid_S8_adjusted[t] = ct.gas_constant * tank.elyte_obj.T/1e6*np.log(X_k_timestep_S8) + g_f_S8
                g_f_solid_Li2S_adjusted[t] = ct.gas_constant * tank.elyte_obj.T/1e6*np.log(X_k_timestep_Li2S) + g_f_Li2S
        ...
        for i in range(num_figs):
            ...
            plt.suptitle(r"$\Delta G_{rxn}$")
            plt.tight_layout(pad=0.1, h_pad=0.1)
            plt.subplots_adjust(wspace=0.2) 

    # The solid dissolution rate
    if plot_flags[6] == 1:
        fig, (ax1,ax2) = plt.subplots(2, 1)
        A_surf_S8 = 4 * np.pi * ((3 * mass_S8) / (4 * np.pi * solids.rho_S8))**(2/3)
        A_surf_Li2S = 4 * np.pi * ((3 * mass_Li2S) / (4 * np.pi * solids.rho_Li2S))**(2/3)
        ax1.plot(time, S_8_disolve_rate*A_surf_S8, label = 'disolve' )
        ax1.plot(time, q_dot[:,SV_idx.elyte_species.index('S8(e)')]*elyte_volume,label = 'react')
        ax1.set_ylabel(r"Consumption/Production [kmol/s]")
        ...
        ax1.set_title(r'$S_8$')
        ax2.plot(time, Li2S_disolve_rate*A_surf_Li2S, label = 'disolve' )
        ax2.plot(time, q_dot[:,SV_idx.elyte_species.index('Li2S(e)')]*elyte_volume,label = 'react')
    ...
    # Plot the rates of each reaction 
    if plot_flags[7] == 1:    
        for i in range(num_figs):
            ...
            fig_manager = plt.get_current_fig_manager()
            fig_manager.window.showMaximized()
            plt.suptitle(r"Bulk Reaction Rates [kmol/m$^3$-s]")
            plt.tight_layout(pad=0.1, h_pad=0.1)
            plt.subplots_adjust(wspace=0.2)
    ...
    # Plot the gibbs Free energy of the mixture
    if plot_flags[9] == 1:
        plt.figure()
        plt.plot(time,gibbs_mixture)
