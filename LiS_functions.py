...
def update_r_avg(bucket_phase, trailing_bookmark):
    '''
    Sets the average radius for each bin. The starting value is the radius of the particles when they
    are nucleating, and the bins in front of where nucleation is occurring have radii that 
    increases by the thickness of a bin
    '''
    r_avg = np.zeros(bucket_phase.n)
    for i in range(int(trailing_bookmark/bucket_phase.thickness),bucket_phase.n-1):
        r_avg[i] = bucket_phase.r_nuc + bucket_phase.thickness*i
    return r_avg
...
def residual(t,SV,SV_dot,resid,user_data):
    ...
    bucket_Li2S.r_nuc = bucket_Li2S.thickness
    bucket_S8.r_avg = update_r_avg(bucket_S8, bm_S8_back)
    bucket_Li2S.r_avg = update_r_avg(bucket_Li2S, bm_Li2S_back)
...
def plot_results(plot_flags, time, N_S8, N_Li2S, bucket_S8, bucket_Li2S, 
    ...
    for j in range(len(N_Li2S_empty[0])):
        shift = int(bm_Li2S_back[j]/bucket_Li2S.thickness)
        clm = int(bm_Li2S_front[j]/bucket_Li2S.thickness)
        spread = clm-shift
        for indx, ele in enumerate(N_Li2S):
            val = ele[j]
            if val != 0:
                destination_row = np.copy(N_Li2S_empty[(spread+shift)-indx])
                destination_row[j] = val
                N_Li2S_empty[(spread+shift)-indx] = np.copy(destination_row)
    N_Li2S = N_Li2S_empty
