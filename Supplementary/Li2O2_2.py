...
from Li2O2_2_functions import bucket, Index_start, residual
import os
import matplotlib.pyplot as plt
...
'''
sim_inputs = np.zeros(n_bucket_Li2O2 + 4)
sim_inputs[:SV_index.Li2O2] = np.zeros(n_bucket_Li2O2)
bin_nuc = 50
sim_inputs[bin_nuc] = 2.1e10
sim_inputs[SV_index.bm_Li2O2_front_2] = bucket_Li2O2.thickness*bin_nuc #+ bucket_Li2O2.thickness/2 # initial nucleation radius of 10 nm
sim_inputs[SV_index.bm_Li2O2_back] = 0

sim_inputs[SV_index.bm_Li2O2_front_2] = t_bucket_Li2O2*(bin_nuc+1)
sim_inputs[SV_index.bm_Li2O2_back_2] = t_bucket_Li2O2*(bin_nuc)
atol = np.multiply(np.ones(n_bucket_Li2O2+4),1e6)
atol[-1] = 1e-8
atol[-2] = 1e-8
atol[-3] = 1e-8
atol[-4] = 1e-8
...
bm_Li2O2_front_2 = sim_outputs[SV_index.bm_Li2O2_front_2]
time = sim_outputs[-1]
...
plt.plot(np.multiply(bucket_Li2O2.r_avg_graph,2), np.multiply(np.divide(V_phase,Capacity),theta*rho_Li2O2*sc_Li2O2*100), linestyle='-', color=plt_clrs[1],linewidth=2)
...
save_fig('PSD_2_CapFrac',folder_name)

fig2 = plt.figure(num=2,figsize=(3,2.25),dpi=400)
plt.plot(np.multiply(bucket_Li2O2.r_avg_graph,2), np.divide(nLi2O2,sum(nLi2O2))*100, linestyle='-', color=plt_clrs[1],linewidth=2)
plt.xlim([0,1e-7])
plt.ylim([0,10])
plt.xticks([0,50e-9,100e-9,150e-9,200e-9],['0','50','100','150','200'],fontsize = 10)
plt.xlabel(r"Particle diameter [nm]",fontsize = 12)
...
ax10.plot(time,bm_Li2O2_front_2,'.')
...
import pandas as pd
fp = "Supplementary/Num_Particles_2.csv"
df  = pd.DataFrame()
df[0] = nLi2O2
